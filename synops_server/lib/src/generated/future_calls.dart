...
import '../future_calls/task_timeout_call.dart' as _iitcwhnf;
...
class FutureCalls extends _is.FutureCallDispatch<_FutureCallRef> {
  ...
  void initialize(
    ...
    var registeredFutureCalls = <String, _is.InvokableFutureCall>{
      'TaskTimeoutCallHandleTimeoutFutureCall':
          TaskTimeoutCallHandleTimeoutFutureCall(),
    };
...
class _FutureCallRef {
  ...
  late final taskTimeoutCall = _TaskTimeoutCallFutureCallDispatcher(
    _invokeFutureCall,
  );
}

class _TaskTimeoutCallFutureCallDispatcher {
  _TaskTimeoutCallFutureCallDispatcher(this._invokeFutureCall);

  final _InvokeFutureCall _invokeFutureCall;

  Future<void> handleTimeout(int taskId) {
    var object = _ih1ht7tw.TaskTimeoutCallHandleTimeoutModel(taskId: taskId);
...
class TaskTimeoutCallHandleTimeoutFutureCall
    extends _is.FutureCall<_ih1ht7tw.TaskTimeoutCallHandleTimeoutModel>
    implements
        _is.InvokableFutureCall<_ih1ht7tw.TaskTimeoutCallHandleTimeoutModel> {
  @override
  _ida.Future<void> invoke(
    _is.Session session,
    _ih1ht7tw.TaskTimeoutCallHandleTimeoutModel? object,
  ) async {
    if (object != null) {
      await _iitcwhnf.TaskTimeoutCall().handleTimeout(
        session,
        object.taskId,
      );
    }
