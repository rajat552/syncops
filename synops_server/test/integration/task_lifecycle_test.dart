...
void main() {
  withServerpod(
    ...
    test('Coordinator creates critical incident, sets up timeline and open task list', () async {
      final task = await endpoints.task.createTask(
        sessionBuilder,
        title: 'Emergency Trauma Supply Delivery',
        description: 'Urgent delivery of blood units to Sector 4 field hospital.',
        latitude: 37.7749,
        longitude: -122.4194,
        severity: 'CRITICAL',
        createdById: 101,
      ...
      final timeline = await endpoints.task.getTaskTimeline(sessionBuilder, task.id!);
    ...
    test('Prevents duplicate concurrent acceptance by second responder', () async {
      final task = await endpoints.task.createTask(
        sessionBuilder,
    ...
    test('Responder advances task through full lifecycle to COMPLETED', () async {
      final task = await endpoints.task.createTask(
        sessionBuilder,
    ...
    test('Rejects unauthorized status updates and invalid state transitions', () async {
      final task = await endpoints.task.createTask(
        sessionBuilder,
    ...
    test('Automated recovery: Simulating timeout reopens task, clears responder, and increments count', () async {
      final task = await endpoints.task.createTask(
        sessionBuilder,
      ...
      final timeline = await endpoints.task.getTaskTimeline(sessionBuilder, task.id!);
      expect(timeline.any((e) => e.type == 'TIMEOUT_AUTO_RECOVERED'), isTrue);
